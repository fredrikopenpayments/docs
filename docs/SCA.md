...
If the ASPSP supports Redirect SCA, the message flow within the payment initiation service is simple. The Payment Initiation Request is followed by an explicit request from TPP to start the authorisation. This is followed by a redirection to the ASPSP SCA authorisation site. 
A status request can be sent by the TPP.
...
If no additional data is needed from the TPP, the ASPSP can start the authorisation process implicitly. This optimisation process results in the following flow. 

In this case, the redirection of the PSU browser session happens directly after the Payment Initiation Response. In addition an SCA status request can be sent by the TPP to follow the SCA process.
...
If an ASPSP supports at least one decoupled SCA method and at least one SCA method that is not decoupled, the flows might be mixed.
Then the ASPSP needs to start the process with the assumption of one specific SCA approach to offer all available SCA methods to the PSU.

If the ASPSP starts the payment initiation flow with a Redirect SCA, the PSU can select the decoupled authentication method on the ASPSP authentication site. This is then transparent for the TPP and has no influence on the transaction flows.

If the ASPSP starts the payment initiation flow with the Embedded SCA, the ASPSP will provide a list of available SCA methods to the PSU via the TPP. 

If the PSU selects an authentication method which requires the Decoupled SCA,  the ASPSP is branching into the Decoupled SCA transaction flow: 
The ASPSP will return the corresponding HTTP header ASPSP-SCA-Approach with value "DECOUPLED" and the current status of the payment initiation, e.g. "ACTC" for correct technical checks, but will return no hyperlink for further action other than the "self" and "status" hyperlink. The next request of the TPP then needs to be the GET Status Request to get the final status of the transaction after having processed the SCA method.
